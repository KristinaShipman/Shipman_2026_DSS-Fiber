...
setwd("/Users/kristinasorokolet/Library/CloudStorage/Box-Box/Wright Lab Operations/Student Folders/Kris/KS_2/Proteomics/GLOBAL/RANDOM_BINS/FEMALES_CTABS/MS9_1P_75_unclus/KEGG_outputs/")
...
all_lines <- readLines("combined_fasta_females.emapper.annotations")
...
ht <- Heatmap(
  ...
  show_row_dend = FALSE,
  column_title = "KEGG pathways - females",
...
png(
  filename = "females_GDH_pathways_imp_clus.png",
  width = 3500,
  height = 3000,
  res = 300
)
...
pheatmap(
  ...
  main = "NADP-GDH pathways (females)",
  name = "Z-score",
...
p_volcano <- ggplot(res_kegg, aes(x = logFC, y = negLog10P)) +
  ...
  labs(
    title = "Volcano Plot: DSS_HFID vs HFID (females)",
    x = "log2 Fold Change",
...
p_box <- p_box + 
  ...
  labs(
    title = "Amino acid metabolizing proteins",
...
ggsave(
  "AA_pathway_proteins_boxplot_sum_imp.png",
  p_box,
  width=5,
  height=5.5,
  dpi=600
...
p_volcano <- ggplot(
  ...
  labs(
    title="Volcano Plot: DSS_HFID vs DSS (females)",
    x="log2 Fold Change",
    y="-log10 adjusted P-value"
  )
...
p_volcano <- ggplot(res_kegg, aes(x = logFC, y = negLog10P)) +
  ...
  labs(
    title = "Volcano Plot: carb pathways DSS_HFID vs HFID (females)",
    x = "log2 Fold Change",
    y = "-log10 adjusted P-value"
  )
...
result <- plot_ko_pathway(
  ...
  meta = meta,
  ko_number = "ko00051",
  pathway_name = "Fructose metabolism - females",
...
result <- plot_ko_pathway(
  ...
  meta = meta,
  ko_number = "ko00052",
  pathway_name = "Galactose metabolism - females",
