...
setwd("/Users/kristinasorokolet/Library/CloudStorage/Box-Box/Wright Lab Operations/Student Folders/Kris/KS_2/Proteomics/GLOBAL/RANDOM_BINS/With_host/males/")
...
ctab_file <- "M_w_host_ms9_1p_r_filtered75.tsv"
...
p_group <- ggplot(df_group_avg,
  ...
  labs(
    x = NULL,
    y = "Relative abundance (%)",
    title = "Host vs microbial protein contribution (males)"
  ) +
...
p_group <- ggplot(df_group_avg,
  ...
  # "ns" text
  annotate("text", x = 3.5, y = 1.08, label = "ns", size = 4, fontface = "italic") +
...
ctab_file <- "M_w_host_ms9_1p_r_filtered75.csv"
